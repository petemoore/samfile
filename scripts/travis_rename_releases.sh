...
if [ -f "${GOPATH}/bin/samfile" ]; then
  mkdir "${GOPATH}/bin/linux_amd64"
  mv "${GOPATH}/bin/samfile" "${GOPATH}/bin/linux_amd64/samfile"
fi
...
export RELEASE_FILE="${TRAVIS_BUILD_DIR}/samfile-${GIMME_OS}-${GIMME_ARCH}${FILE_EXT}"
mv "${GOPATH}/bin/${GIMME_OS}_${GIMME_ARCH}/samfile${FILE_EXT}" "${RELEASE_FILE}"
